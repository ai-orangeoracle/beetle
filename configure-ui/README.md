...
2. **The device powered on and reachable.**  
   - **First use / not yet on your WiFi:** The device will open a **WiFi hotspot** with SSID **Beetle** (no password). Your phone or PC must **connect to this hotspot**; then open **http://192.168.1.4** (matches firmware SoftAP address).
...
3. In the browser open **http://192.168.1.4** (port 80; firmware SoftAP address).

Only the device is on that hotspot; the firmware uses 192.168.1.4 for the SoftAP.
...
4. **Enter the device address in the page**  
   ...
   - When connected to the device’s hotspot, enter **http://192.168.1.4** (firmware SoftAP address); when on the same LAN, enter the router-assigned IP.
...
**If the page says it cannot reach the device:** Check that (1) the device is powered on, (2) you are connected to the **Beetle** hotspot or the **same LAN** as the device, and (3) the device address you entered is correct—use **http://192.168.1.4** when on the hotspot, or the device’s LAN IP when on the same LAN.
