...
本文档面向**对接设备 HTTP API 的开发者**（如自建配置页、脚本或第三方集成）。设备固件仅提供 HTTP API，**不**内嵌配置页；配置页由外置前端（如本仓库 `configure-ui` 或 GitHub Pages 部署）实现。用户连接设备热点或与设备同网后，在配置页中填写**设备地址**（推荐 **http://beetle.local/**，不可用时填 **http://192.168.4.1** 或路由器分配的 IP）即可调用下述接口。

## 网络与访问

- **SoftAP**：设备上电后开启热点，SSID 固定为 **Beetle**（无密码）。连接该热点后，推荐访问 **http://beetle.local/**；若不可用则使用设备 IP **http://192.168.4.1**（与固件一致）。
- **STA**：若已配置 WiFi 并连接用户路由器，同一 HTTP 服务在 STA 网段也可访问。推荐 **http://beetle.local/**（mDNS）；若不可用则使用路由器分配的 LAN 地址。
...
- **推荐**：优先使用 **http://beetle.local/**（连接设备热点或同 LAN 时通常可用，与固件 mDNS 一致）。
- 连接设备热点 **Beetle** 时若 beetle.local 不可用：使用 **http://192.168.4.1**（固件 SoftAP 固定地址）。
- 已连 STA 且与设备在同一 LAN 时若 beetle.local 不可用：使用路由器分配给设备的 IP。
