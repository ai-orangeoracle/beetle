...
设备固件仅提供 HTTP API，**不**内嵌配置页；配置页由外置前端（如独立仓库或 GitHub Pages）实现，用户连接设备热点或同网段后，在页面中填写**板子 IP** 即可调用下述接口。

## 网络与访问

- **SoftAP**：设备上电后开启热点，SSID 固定为 `PocketCrayfish`（无密码）。连接该热点后，设备 IP 为 **192.168.4.1**。
- **STA**：若已配置 WiFi 并连接用户路由器，同一 HTTP 服务在 STA 网段也可访问，设备 IP 为路由器分配的 LAN 地址，或 **http://pocketcrayfish.local**（mDNS 已支持）。
...
- 连接设备热点 `PocketCrayfish` 时：使用 **192.168.4.1**。
- 已连 STA 且与设备在同一 LAN 时：使用路由器分配给设备的 IP，或 **http://pocketcrayfish.local**（mDNS 已支持）。
