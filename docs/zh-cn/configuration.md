...
**推荐**：无论是否已配网，优先在浏览器打开 **http://beetle.local/**（mDNS；连接设备热点或与设备同局域网时通常可用）。若无法访问，再使用设备 IP **http://192.168.4.1**（与固件一致，仅连接设备热点时为 192.168.4.1）。

### 未配网（首次使用）

1. 设备上电后开启热点，SSID 为 **Beetle**（无密码）
2. 手机或电脑连接该热点
3. 浏览器打开 **http://beetle.local/**；若打不开再打开 **http://192.168.4.1**（端口 80，无需输入端口号）

此时仅设备自身在该热点下；固件 SoftAP 地址为 192.168.4.1，与家中路由器不冲突。

### 已连 WiFi（配网完成后）

设备连接家中路由器后，只要手机/电脑与设备在同一局域网：

- **http://beetle.local/**（推荐，mDNS，与网段无关）
- 若 beetle.local 不可用，再使用路由器分配给设备的 IP
...
1. **从设备打开**：连接设备热点或与设备同网后，在浏览器访问 **http://beetle.local/**（推荐）或 **http://192.168.4.1**，由设备提供或跳转到配置页。
2. **在线打开**：访问 **https://ai-orangeoracle.github.io/beetle/**（若仓库配置了自定义域名则以该域名为准）。在线版仍需要已烧录固件的设备且浏览器与设备在同一网络；打开后需在页面中填写设备地址（**http://beetle.local/** 或 **http://192.168.4.1**），才能读写配置。
...
- **GET /api/health**：无需配对码，返回 WiFi 状态、入站/出站队列深度、最近错误摘要及 **metrics** 快照（消息进/出、LLM/tool 调用与错误、WDT feed、按 stage 错误计数等，无敏感信息）。调用方式：在浏览器或脚本中请求 **http://beetle.local/api/health**（或设备 IP 替换 `beetle.local`）。便于运维巡检与优化前后对比。
