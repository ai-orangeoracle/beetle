...
- **浏览器配网**：没配网时板子会开一个叫 **Beetle** 的热点（无密码），手机连上后打开 **http://192.168.1.4** 就能配置。配好 WiFi 后用路由器分配的 IP 访问，配对码保护写操作。
...
**第一次用**：板子上电后会开一个叫 **Beetle** 的热点，手机连上后打开 **http://192.168.1.4** 填 WiFi 和配对码就行。
...
| 浏览器配网   | 热点 Beetle → http://192.168.1.4；连上 WiFi 后用路由器分配的 IP，配对码保护写操作                                                                                                                                                                                                             |
