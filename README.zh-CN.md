...
**配对码保护**
- 首次使用时设置配对码，用于保护所有配置修改操作
- 配对码在浏览器本地保存，**1 小时后自动过期**，需重新输入
- 建议使用 6 位以上随机字符，避免使用简单密码

**跨域访问**
- 可通过公网域名(如 http://beetle.uno)访问局域网设备
- 所有状态修改操作受 CSRF 保护，防止恶意网站伪造请求
- 首次访问时自动获取安全令牌，无需手动操作

**使用建议**
- 仅在可信网络环境下使用配置页面
- 不要在公共场所或不安全的 WiFi 下进行配置
- 定期更换配对码以提高安全性
- 配置完成后可断开设备热点，仅通过路由器 IP 访问
