...
fn detect_partition_filename(files: &[PathBuf]) -> Option<String> {
    files.iter().find_map(|file| {
        fs::read_to_string(file).ok().and_then(|content| {
            content.lines().find_map(|line| {
                let line = line.trim();
                line.strip_prefix("CONFIG_PARTITION_TABLE_CUSTOM_FILENAME=")
...
pub fn build() -> Result<EspIdfBuildOutput> {
    ...
    let partition_filename = detect_partition_filename(&sdkconfig_defaults);

    // Copy custom partition table from workspace to out dir when ESP-IDF resolves it relative to out/.
    // Always overwrite when src exists so a clean build is not required when changing partitions.csv.
    if let Some(ref name) = partition_filename {
        let src = workspace_dir.join(name);
        let dst = out_dir.join(name);
        if src.exists() {
            fs::copy(&src, &dst)
                .with_context(|| anyhow!("copy partition table {} -> {}", src.display(), dst.display()))?;
        }
    }
